...
def signup(request):
    if request.method == "GET":
        ...
        login(request, user)
        return redirect(reverse("onlinelibrary:homeview"))
...
def viewitem(request):
    items = Book.objects.filter(user=request.user)  
    context ={'items': items }
    return render(request, 'webapp/booklist.html',context)


def itemdetail(request, item_id):
    item = get_object_or_404(Book, id=item_id, user=request.user)
